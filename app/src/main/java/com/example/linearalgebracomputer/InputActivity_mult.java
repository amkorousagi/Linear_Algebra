...
public class InputActivity_mult extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_input_mult);
        ...
        //행 스피너 인자에 따라 행렬 유동적으로 조정
        final Spinner spinner_field3 = (Spinner)findViewById(R.id.spinner_field3);
        String[] str3 = getResources().getStringArray(R.array.spinnerArray3);
        final ArrayAdapter<String> adapter3= new ArrayAdapter<String>(getApplicationContext(),R.layout.spinner_item,str3);
        adapter3.setDropDownViewResource(R.layout.support_simple_spinner_dropdown_item);
        spinner_field3.setAdapter(adapter3);
        spinner_field3.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            public void onItemSelected(AdapterView<?> adapterView, View view, int i, long l) {
                if (spinner_field3.getSelectedItemPosition() > 0) {
                    colb = parseInt(spinner_field3.getSelectedItem().toString());
                    Log.v("알림", spinner_field3.getSelectedItem().toString() + "is selected");
                    for (int s = 0; s < idNumbers.length; s++) {
                        for(int j=0; j<idNumbers[0].length; j++) {
                            EditText input2 = (EditText) findViewById(idNumbers2[s][j]);
                            input2.setVisibility((View.INVISIBLE));
